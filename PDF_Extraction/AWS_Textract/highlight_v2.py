...
def clean_target_text(target_text):
    """
    Cleans the target text by removing entire <HEADING> blocks and {['TITLE EXTRACT']} placeholder.
    """
    # Remove entire <HEADING>...</HEADING> blocks, including the enclosed text
    cleaned_text = re.sub(r'<HEADING>.*?</HEADING>', '', target_text, flags=re.DOTALL)
...
def main():
    ...
    output_path = 'test2_10_highlighted_v3.png'  # Path to save the highlighted image
