...
def json_to_text(response: Dict[str, Any]) -> str:
    ...
    for idx in sorted_centroid_indices:
        ...
        for block_info in column_blocks:
            ...
            if block_type == 'LINE':
                ...
                overlaps_table = any(bbox_overlap(line_bbox, table_bbox) for table_bbox in table_bboxes)
...
def final_output(response: Dict[str, Any], pdf_filepath: str, page_number: int) -> str:
    json_text = json_to_text(response)
...
def extract_entire_pdf(pdf_filepath: str, client: boto3.client, output_filepath: str) -> None:
    ...
    for page_number in range(num_pages):
        ...
        accumulated_text.append(final_output(response, pdf_filepath, page_number))
        print("PAGE DONE!!\n")
